...
class Clue(RecourseMethod):
    ...
    def _load_vae(self):
        ...
        try:
            vae.load(os.path.join(path, f"theta_best_{self._width}_{self._depth}_{self._latent_dim}_{self._lr}_{self._early_stop}.dat"))
            print("VAE LOADED: ", vae.model)
        except FileNotFoundError as e:
            print(
                "Could not load best model, maybe because no model has been good! Loading last model",
                e,
            )
            vae.load(os.path.join(path, f"theta_last_{self._width}_{self._depth}_{self._latent_dim}_{self._lr}_{self._early_stop}.dat"))
        #print("VAE LOADED: ", vae.model)
        return vae

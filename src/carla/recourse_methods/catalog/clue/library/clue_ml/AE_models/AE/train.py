...
def train_VAE(
    ...
    for i in range(epoch, nb_epochs):
        ...
        if vlb_dev[i] > best_vlb:
            best_vlb = vlb_dev[i]
            best_epoch = i
            net.save(os.path.join(models_dir, f"theta_best_{net.width}_{net.depth}_{net.latent_dim}_{net.lr}_{early_stop}.dat"))

        if early_stop is not None and (i - best_epoch) > early_stop:
            break

    net.save(models_dir + f"/theta_last_{net.width}_{net.depth}_{net.latent_dim}_{net.lr}_{early_stop}.dat")
    toc0 = time.time()
